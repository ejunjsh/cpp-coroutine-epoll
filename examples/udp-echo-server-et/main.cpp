...
Task<void> handle_datagram(EventLoop& loop, UdpSocket& socket, ThreadPool& business_pool,
                           std::string input, coro_epoll::UdpEndpoint endpoint) {
    ...
    co_await socket.async_send_to(output.data(), output.size(), endpoint);
}
...
Task<void> udp_echo_loop(EventLoop& loop, UdpSocket& socket, ThreadPool& business_pool) {
    try {
        ...
        while (true) {
            // ET recv: drain all available datagrams in one batch
            auto results = co_await socket.async_recv_from_et(buffer.data(), buffer.size());

            for (auto& received : results) {
                std::string input{buffer.data(), received.size};
                loop.spawn(handle_datagram(loop, socket, business_pool,
                                           std::move(input), std::move(received.endpoint)));
            }
        }
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "udp echo server (ET mode) listening on 0.0.0.0:" << port
