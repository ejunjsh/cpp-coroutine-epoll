...
Task<void> handle_client(EventLoop& loop, TcpSocket socket, ThreadPool& business_pool) {
    try {
        ...
        while (true) {
            // ET read: drain the socket buffer in one batch
            const std::size_t count = co_await socket.async_read_et(buffer.data(), buffer.size());
            if (count == 0) {
            ...
            // Write stays LT (no async_write_et) to avoid ET write race
...
Task<void> accept_and_handle(EventLoop& loop, TcpServer& server, ThreadPool& business_pool) {
    try {
        while (true) {
            // ET accept: drain the accept queue in one batch
            std::vector<TcpSocket> sockets = co_await server.async_accept_et();

            for (auto& socket : sockets) {
                loop.spawn(handle_client(loop, std::move(socket), business_pool));
            }
        }
    } catch (const std::exception& error) {
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "tcp echo server (ET mode) listening on 0.0.0.0:" << port
